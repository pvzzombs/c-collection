...
void BigInt_add_int_impl(BigInt_limb_t *, BigInt_limb_t, BigInt_limb_t *, int, int);
void BigInt_add_int_unsigned(BigInt *, BigInt *, BigInt_limb_t);
...
void BigInt_multiply_int_impl(BigInt_limb_t *, BigInt_limb_t, BigInt_limb_t *, int, int);
void BigInt_multiply_int_unsigned(BigInt *, BigInt *, BigInt_limb_t);
...
void BigInt_divmod_unsigned(BigInt *, BigInt *, BigInt *, BigInt *);
void BigInt_divmod(BigInt *, BigInt *, BigInt *, BigInt *);
...
void BigInt_init_globals() {
  BigInt_init_zero(&ZERO);
  BigInt_init_one(&ONE);
  BigInt_init_two(&TWO);
}

void BigInt_destroy_globals() {
  BigInt_destroy(&ZERO);
  BigInt_destroy(&ONE);
  BigInt_destroy(&TWO);
}
...
void BigInt_add_int_unsigned(BigInt * sum, BigInt * addend1, BigInt_limb_t addend2) {
  if (sum->allocSize > addend1->internalSize) {
    sum->internalSize = addend1->internalSize + 1;
  } else {
    BIGINT_FREE(sum->internalRepresentation);
  ...
  BigInt_add_int_impl(addend1->internalRepresentation, addend2, sum->internalRepresentation, addend1->internalSize, sum->internalSize);
  BigInt_remove_leading_zeroes(sum);
}
...
void BigInt_subtract(BigInt * difference, BigInt * minuend, BigInt * subtrahend) {
  ...
  if (difference->allocSize >= minuend->internalSize + allowance) {
    difference->internalRepresentation[minuend->internalSize + allowance - 1] = 0; /* This is not necessary */
...
void BigInt_multiply_int_impl(BigInt_limb_t * multiplicand, BigInt_limb_t multiplier, BigInt_limb_t * product, int multiplicandLen, int productLen) {
  BigInt_limb_wide_t carryM;
  BigInt_limb_wide_t carryA;
  BigInt_limb_wide_t productDigit;
  BigInt_limb_wide_t sumDigit;
...
void BigInt_multiply_int_unsigned(BigInt * product, BigInt * multiplicand, BigInt_limb_t multiplier) {
  int i;
  ...
  BigInt_multiply_int_impl(multiplicand->internalRepresentation, multiplier, product->internalRepresentation, multiplicand->internalSize, product->internalSize);
  BigInt_remove_leading_zeroes(product);
}
...
void BigInt_divide_with_remainder_optimize_bump_impl(BigInt_limb_t * dividend, BigInt_limb_t * divisor, BigInt_limb_t * quotient, BigInt_limb_t * remainder, int dividendLen, int divisorLen, int quotientLen, BigInt_Bump_Allocator * bmp) {
  int remainderLen = divisorLen + 1;
  int i, j;
  BigInt_limb_wide_t qDigit1, qDigit2, dvsrDigit, qhat, base = BIGINT_BASE;
  BigInt_limb_t * tempHolder;
...
void BigInt_divide_int_impl(BigInt_limb_t * dividend, BigInt_limb_t divisor, BigInt_limb_t * quotient, int dividendLen) {
  int i;
  BigInt_limb_wide_t r = 0, tempq;
  BigInt_limb_t q;
  
  for (i = dividendLen - 1; i >= 0; i--) {
    r = r * BIGINT_BASE + dividend[i];
    tempq = r / divisor;
    q = tempq;
    quotient[i] = q;
    tempq *= divisor;
...
void BigInt_divmod_unsigned(BigInt * quotient, BigInt * remainder, BigInt * dividend1, BigInt * divisor) {
  int i, s = 0;
  BigInt_limb_t * d;
  BigInt_limb_t * newDividend;
  BigInt_limb_t * newDivisor;
  ...
  if (BigInt_cmp_len(dividend1, divisor) < 0 || BigInt_is_zero_impl(divisor->internalRepresentation, divisor->internalSize)) {
    ...
    if (remainder != NULL) {
      BigInt_copy(remainder, dividend1);
    }
    return;
  }
  
  BigInt_Bump_Allocator_init(&bmp, 1 
    + dividend1->internalSize + 1 
    + dividend1->internalSize + 1 
    + divisor->internalSize 
    + divisor->internalSize + 1
    + divisor->internalSize + 1
    + 1);
  ...
  newDividend = BigInt_Bump_Allocator_alloc(&bmp, dividend2.internalSize); s += dividend2.internalSize;
  newDivisor = BigInt_Bump_Allocator_alloc(&bmp, divisor->internalSize); s += divisor->internalSize;
  rem = BigInt_Bump_Allocator_alloc(&bmp, divisor->internalSize + 1); s += divisor->internalSize + 1;
  ...
  BigInt_multiply_optimize_impl(dividend2.internalRepresentation, d, newDividend, dividend2.internalSize - 1, 1, dividend2.internalSize);
  BigInt_multiply_optimize_impl(divisor->internalRepresentation, d, newDivisor, divisor->internalSize, 1, divisor->internalSize);
  BigInt_divide_with_remainder_optimize_bump_impl(newDividend, newDivisor, quotient->internalRepresentation, rem, dividend2.internalSize, divisor->internalSize, quotient->internalSize, &bmp);
  BigInt_remove_leading_zeroes(quotient);
  
  if (remainder != NULL) {
    if (remainder->allocSize >= divisor->internalSize + 1) {
      remainder->internalSize = divisor->internalSize + 1;
    } else {
      BIGINT_FREE(remainder->internalRepresentation);
      remainder->internalRepresentation = BigInt_alloc_helper(&(remainder->allocSize), divisor->internalSize + 1);
      remainder->internalSize = divisor->internalSize + 1;
    }
...
void BigInt_divide(BigInt * quotient, BigInt * dividend1, BigInt * divisor) {
  ...
  if (BigInt_cmp_unsigned(dividend1, divisor) < 0 || BigInt_is_zero_impl(divisor->internalRepresentation, divisor->internalSize)) {
    if (quotient->allocSize >= 1) {
      quotient->internalRepresentation[0] = 0;
      quotient->internalSize = 1;
    } else {
...
void BigInt_divmod(BigInt * quotient, BigInt * remainder, BigInt * dividend1, BigInt * divisor) {
  int i, s = 0;
  BigInt_limb_t * d;
  BigInt_limb_t * newDividend;
  BigInt_limb_t * newDivisor;
...
void BigInt_set_from_string_with_small(BigInt * b, char * str) {
  ...
  BigInt_init(&temp);
  for (i = 0; i < len; i++) {
    BigInt_multiply_int_unsigned(&out, &temp, 10);
    BigInt_add_int_unsigned(&temp, &out, str[i] - '0');
  }
  BigInt_copy(b, &temp);
  BigInt_destroy(&out);
  BigInt_destroy(&temp);
...
void BigInt_set_from_string_impl(BigInt * b, char * str) {
  ...
  BigInt_add_int_unsigned(b, &out, BigInt_atoi_impl_with_range(str, 0, firstLen - 1));

  for (i = firstLen; i < len; i+=BIGINT_BASE_DIGITS) {
    BigInt_multiply_int_unsigned(&out, b, BIGINT_BASE_10_BIG);
    BigInt_add_int_unsigned(b, &out, BigInt_atoi_impl_with_range(str, i, i + BIGINT_BASE_DIGITS - 1));
  }
  BigInt_destroy(&out);
}
...
void BigInt_multiply_small_u(BigInt * out, BigInt * a, BigInt_limb_t b) {
  ...
  BigInt_multiply_int_unsigned(&temp, a, b);
...
void BigInt_set_from_base_string_unsigned(BigInt * z, char * str, BigInt_limb_t base) {
  ...
  for (i = 0; i < len; i++) {
     ...
     * temp = b + str[i]
     * */
    BigInt_multiply_int_unsigned(&b, &temp, base);
    BigInt_add_int_unsigned(&temp, &b, BigInt_to_int_from_char(str[i]));
  }
