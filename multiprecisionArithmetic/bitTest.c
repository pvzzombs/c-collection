...
int main() {
  ...
  BigInt_init_from_string_unsigned(&a, "123456789012345678901234567890");
  BigInt_init_none(&product);
  BigInt_init_two(&two);
  BigInt_init_from_string_unsigned(&b, "1");
  BigInt_init_none(&ShiftLeftByTwo);
  
  for (i = 0; i < 100; i++) {
    BigInt_multiply_u(&b, &b, &two);
    BigInt_divide_unsigned(&product, &a, &b);
    s = BigInt_to_string_unsigned(&b);
    printf("divider: %s %d\n", s, i + 1);
    free(s);
    BigInt_print_internal(&product);
    BigInt_set_from_string_unsigned(&ShiftLeftByTwo, "123456789012345678901234567890");
    BigInt_shift_right_bit(&ShiftLeftByTwo, i + 1);
    BigInt_print_internal(&ShiftLeftByTwo);
    
    if (BigInt_cmp_unsigned(&product, &ShiftLeftByTwo) != 0) {
      printf("Mismatch!\n");
      break;
    }
  }
