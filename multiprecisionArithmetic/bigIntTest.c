...
void randomizedMultiplyKaratsubaBumpNLimbsMaxFill() {
  BigInt a,b,schoolbook,karatsuba;
  int i, lenA, lenB;
  char * a_str;
  char * b_str;
  ...
  for (i = 0; i < 100; i++) {
    ...
    if (lenB < 64) lenB = 64;
    /* printf("Lens: %d, %d\n", lenA, lenB); */
    BigInt_init_fill_max(&a, lenA);
    BigInt_init_fill_max(&b, lenB);
    BigInt_init_zero_limb(&schoolbook, lenA + lenB);
    BigInt_init_zero_limb(&karatsuba, lenA + lenB);
    BigInt_multiply_unsigned(&schoolbook, &a, &b);
    BigInt_multiply_karatsuba_bump_unsigned(&karatsuba, &a, &b);
...
void additionTestSigned() {
  BigInt num1, num2, ans1, ans2;
  int i, len1, len2;
  for (i = 0; i < 100; i++) {
    len1 = rand() % 100 + 1;
    len2 = rand() % 100 + 1;
